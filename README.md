...
Follow these steps to publish `flutter_mockkit` to pub.dev:

### 1. Prerequisites

- Dart SDK ≥ 3.0 installed.
- A Google account linked to [pub.dev](https://pub.dev).
...
| Issue | Fix |
|---|---|
| Missing `LICENSE` | Add an `MIT` or `BSD` `LICENSE` file. |
| Low pub score | Add doc-comments (`///`) to all public APIs. |
| Missing `example/` | Ensure `example/main.dart` exists. |
...
3. Run `dart pub publish --dry-run` again.
